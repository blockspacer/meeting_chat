...
public:
    ResponseError() {}
    ResponseError(NetworkErrorType errorType_, int64_t code_, const std::string &description_)
...
public:
    NetworkRequest(const std::string &host, const std::string &path, NetworkMethod method, const std::string& pluginName)
        : _host(host)
        , _path(path)
        , _method(method)
