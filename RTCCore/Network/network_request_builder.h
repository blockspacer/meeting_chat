...
public:
    ...
    NetworkRequestBuilder(const std::string& host, const std::string& path, NetworkMethod method, const std::string& pluginName)
        : _host(host)
        , _path(path)
        , _method(method)
        , _pluginName(pluginName)
    ...
    static std::shared_ptr<NetworkRequestBuilder> create(const std::string& host,
                                                         const std::string& path,
                                                         NetworkMethod method,
                                                         const std::string& pluginName)
    {
        auto builder = std::make_shared<NetworkRequestBuilder>(host, path, method, pluginName);
        return builder;
    }
    ...
    std::shared_ptr<NetworkRequest> build() {
        std::shared_ptr<NetworkRequest> request = std::make_shared<NetworkRequest>(_host, _path, _method, _pluginName);
...
private:
    std::function<void(const std::shared_ptr<NetworkResponse>& response)> transformCallback(UNIVERSAL_RESPONSE_CALLBACK callback)
    {
        auto callbackWrapper = [callback](const std::shared_ptr<NetworkResponse>& response) {
            std::shared_ptr<ResponseError> error = std::make_shared<ResponseError>(NetworkErrorType::NoError,
                                                                                   response->code,
